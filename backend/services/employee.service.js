...
class employeeLogics{
    ...
    modifyResignation(payload){
        console.log(payload)
        return ResignInfo.findOneAndUpdate({_id:payload.resignationId},{$set:{status:payload.approved?'true':'false'}},{new:true})
    }
    ...
    addUserResponse=async(payload,Id)=>{
        ...
        if(!userRespo){
            ...
            return await UserResponse.create({employeeId:Id,responses:payload.responses})
        }
